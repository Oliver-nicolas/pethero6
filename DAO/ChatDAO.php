...
class ChatDAO implements IChatDAO
{
    ...
    function SearchByOwner($ownerId)
    {
        try {
            ...
            $query = "SELECT c.*, (SELECT m.date FROM messages m WHERE m.chatId = c.Id ORDER BY m.date DESC LIMIT 1) ultimo_msg FROM " . $this->tableName . " c WHERE c.ownerId = :ownerId ORDER BY ultimo_msg DESC";
            $parameters["ownerId"] = $ownerId;
    ...
    function SearchByKeeper($keeperId)
    {
        try {
            ...
            $query = "SELECT c.*, (SELECT m.date FROM messages m WHERE m.chatId = c.Id ORDER BY m.date DESC LIMIT 1) ultimo_msg FROM " . $this->tableName . " c WHERE c.keeperId = :keeperId ORDER BY ultimo_msg DESC";
            $parameters["keeperId"] = $keeperId;
